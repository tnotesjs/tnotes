...
**复核修正（第 9 节）**：复核方提出 2 条问题（[P1] 嵌套段落删除落点、[P2] frontmatter 注释被当成标题），
都已修复并补测；同时补齐了「表格边界删除、嵌套容器、重做」三项验证。
...
- 复核修正后（按要求**只重跑受影响项**）：单测 `blockBoundaryCaret` + `sourceFolding` **41/41**；
  E2E `e2e-boundary-delete` **37/37**，并连带重跑 `e2e-markdown-input`、`e2e-block-boundary-navigation`、
  `e2e-source-folding`、`e2e-source-unicode-highlight`、`e2e-source-selection-background` **5/5 通过**。
