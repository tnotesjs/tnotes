...
**复核修正（第 9 节）**：复核方共提出 6 条问题，都已修复并补测：
[P1] 嵌套段落删除落点；[P2] frontmatter 注释被当成标题；[P2] 围栏闭合不能无条件剥离容器前缀；
[P2]「顶层标题」不等于「必须顶格」；[P2] 列表项内闭合围栏没扣除容器缩进；
[P2] 离开引用容器后未闭合围栏没有结束。
同时补齐了「表格边界删除、嵌套容器、重做」三项验证。修复范围只限折叠扫描器与删除落点两处。
...
- **setext 标题不参与折叠**：本轮只认 ATX（`#`..`######`）。`===` / `---` 下划线式标题在源码视图里
  不会成为折叠起点（可视化视图里的章节折叠只认 heading 节点，两者对 setext 都不折叠 —— 行为一致）。
...
已补测（原计划要求、复核指出不能只写"未验证"）：

- **表格边界删除**：单测 + E2E 都覆盖（`表格后` 场景：先落表格块后边界 → 再删整表 → 撤销 → 重做）。
- **嵌套容器**：引用内代码块（E2E 真实按键 + 单测）、列表项内空段落与代码块（单测）、
  提示块内代码块（单测）；E2E 还断言"删的是容器内那块、容器外的块原封不动"。
- **重做**：每个剧本在撤销之后都补了 `Cmd+Shift+Z` 重做断言（表格、嵌套、四种块类型都有）。

仍需**人工验证**（无法在自动化里可信复现）：

- **真实 OS 长按自动重复速率**：单测用 `repeat: true` 的合成事件、E2E 用两次连续按键覆盖；
  没有按硬件重复速率验证。
- macOS 上 `fn+Backspace`（前向删除）在**真机键盘**上的按键映射；E2E 用的是 Playwright 的
  `Delete`（即前向删除语义）。

其余未覆盖：图片"有描述"时的边界场景（由 `e2e-image-caption` 覆盖描述输入，边界导航不接管其按键）；
`deskRawBlock` 的其它形态（图表 / 纯 HTML）未单列，但走的是同一条 `isBoundaryStopBlock` 判据。
...
- 首次收尾：单测 `181 文件 / 1640 条` 全部通过；E2E regression **全量 43/43** 通过（157.1s）。
- 复核修正后（按要求**只重跑受影响项**）：
  - 删除落点那一轮：单测 `blockBoundaryCaret` + `sourceFolding` **41/41**；
    E2E `e2e-boundary-delete` **37/37**，连带 `e2e-markdown-input`、`e2e-block-boundary-navigation`、
    `e2e-source-folding`、`e2e-source-unicode-highlight`、`e2e-source-selection-background` **5/5**。
  - 折叠扫描器第一轮（第 9.3 / 9.4 节）：单测 `sourceFolding` **23/23** +
    `sourceFoldingParse` **4/4** + 注册表 **18/18**；E2E `e2e-source-folding` **17/17**。
  - 折叠扫描器第二轮（第 9.5 / 9.6 节，容器边界）：单测 `sourceFolding` **27/27** +
    `sourceFoldingParse` **6/6** + 注册表 **18/18**；E2E `e2e-source-folding` **17/17**；
    `lint` 0 error、`typecheck` 通过、根目录 `format:check` 通过。
  - 未改动的删除逻辑没有重跑（按复核要求不再无差别跑全量）。
...
### 9.1 [P1] 嵌套段落的删除落点会跳到容器外 —— 已修

**问题**：`edgeBoundaryTargetForDelete` 用 `$head.before(1)` / `after(1)`，取的是**顶层祖先**的边界，
不是当前段落所在层级的相邻节点。复核给的复现（外层代码块 A + 引用里的内部代码块 B，
光标在引用内空段落开头按 Backspace）会返回 A。

**修复**：改成 `$head.before($head.depth)` / `after($head.depth)`（段落自己那一层）。
容器内没有相邻特殊块时返回 null（交给 PM 默认），**绝不跳到容器外**。

**验证**：

- 单测新增 4 条（`blockBoundaryCaret.test.ts`，现 **22/22**）：引用内代码块（Backspace）、
  列表项内空段落 + 代码块（Delete 前向）、提示块内代码块（Backspace）、
  容器内没有相邻特殊块时不接管（不跳到容器外）。
- **反向验证**：把层级改回 `level = 1`（修复前写法）后，这 4 条**全部失败**；改回后 22/22 通过。
- E2E 新增 `引用内嵌套` 场景（真实按键）：第一下落点是**内部**代码块
  （探针 `boundaryKind: "code_block"`、`blockKind: "blockquote"`，内容逐字未变），
  第二下删掉的只有内部块 —— 容器外的 `const c = 3`、`表后段` 与顶层块清单原封不动；
  随后撤销、重做各验一次。

### 9.2 [P2] frontmatter 注释被当成标题 —— 已修

**问题**：`sourceFolding.ts` 按行扫标题，没有跳过 frontmatter，`---\n# metadata comment\nid: x\n---`
会多出一个假的章节范围，「全部折叠标题」会去折元数据。

**修复**（三处，都是这次一并收口的"哪些 `#` 才算章节标题"）：

1. `frontmatterEndLine()`：首行独占 `---` 且能找到闭合行（`---` 或 `...`）时整段跳过；
   未闭合就当普通正文（不吞掉整篇）。
2. **只认顶格 ATX 标题**：与可视化编辑器只收集文档级标题（`collectDocHeadings` 只看顶层）一致；
   `> # x`、`  # x` 这类容器/缩进标题不参与章节，避免章节范围伸到容器外。
3. **围栏允许容器前缀**（`stripContainerPrefix`：引用 `> `、列表 `- ` / `1. `，可嵌套）：
   引用/列表里的代码围栏能被识别成整块，块里的 `#` 不再变回"外部章节"；
   另外跟踪 `:::` 容器（提示块 / 代码组）的嵌套深度，容器里的 `#` 也不参与章节。

**验证**（`sourceFolding.test.ts` **19/19**，新增 8 条）：

- 复核给的原例：`# metadata comment` 不再是章节，只有 `# Real` 是（`headingFoldTargetLines('fold-all')`
  只返回第 6 行）。
- `frontmatterEndLine`：闭合 / `...` 闭合 / 未闭合 / 无 frontmatter 四种。
- frontmatter 之后的围栏照常折。
- 引用里的围栏代码块（块内含 `# 引用里的伪标题`）折整块，且不产生外部章节；列表项里的同样。
- 提示块容器里的 `#` 不参与章节。
- 顶格之外的 `#`（`> # x`、`  # x`）不算章节。
- `stripContainerPrefix` 的嵌套引用 / 列表 / 4 空格缩进边界。
- **反向验证**：临时关掉 frontmatter 跳过 → 复核那两个用例失败；恢复后 19/19。

### 9.3 [P2] 围栏闭合不能无条件剥离容器前缀 —— 已修

**问题**：闭合判定先用 `stripContainerPrefix(line)` 再比围栏，于是**代码内容**里的 `> ``` ` /
`- ``` ` / `1. ``` ` 会被剥成"闭合围栏"：围栏提前结束，后面的代码行被当成正文，代码里的 `#`
变成章节，真正的标题反而丢了。

**修复**：闭合判定回到**开栏时的容器上下文**（`lineContext` + 列表项内容列栈）：

- 记录开栏时的 `quoteDepth` 与「容器内容列」；
- 只有**同一引用深度**、且本身**不是列表项标记**、缩进不超过容器内容列 +3 的行，才可能是闭合围栏；
- 剥容器前缀只用于**开栏识别**（`> ``` ` / `- ``` ` 仍然是合法的开栏行），不再用于闭合判定。

### 9.4 [P2]「顶层标题」不等于「必须顶格」—— 已修

**问题**：上一版把标题规则收成"必须顶格"，于是 `  ## Heading`（2 个前导空格，CommonMark 合法的
文档级 ATX 标题）不再参与折叠 —— 这是把原本支持的行为改坏了。

**修复**：恢复 0–3 个前导空格（`^ {0,3}#{1,6}(?:[ \t]|$)`），是否属于容器改由**解析上下文**判断：
逐行算出「引用深度 + 列表项内容列栈 + `:::` 容器深度」，三者都为空才算文档级标题。
列表项栈按"非空行缩进 < 栈顶 → 出项"维护，所以**容器结束后标题识别自动恢复**。

**一致性用实际解析结果验证**（不再从 `collectDocHeadings` 推导）：
新增 `markdown/sourceFoldingParse.test.ts`（用可视化编辑器自己的解析器，4/4）：

| 源码                                      | 可视化解析出来的文档级标题                           |
| ----------------------------------------- | ---------------------------------------------------- |
| `  ## 缩进两格的标题`                     | `## 缩进两格的标题` ✓（所以扫描器必须接受 0–3 空格） |
| `> # 引用里的标题` / `- # 列表项里的标题` | 无 ✓                                                 |
| frontmatter 里的 `# metadata comment`     | 无（只有 `# Real`）✓                                 |
| 代码围栏里的 `# 伪标题`                   | 无（只有 `# Real`）✓                                 |

**回归**（`sourceFolding.test.ts` 现 **23/23**，新增/改写 4 条）：

- 顶层围栏里的 `> ``` `、`- ``` `、`1. ``` ` 都只是代码内容：围栏范围到真正的闭合行，
  代码里的 `#` 不是章节，真正的 `# Real` 正常成为章节；
- 真正的引用 / 列表里的围栏照常整块折叠；
- 容器结束后恢复正常标题识别（列表项里的 `#` 不参与 → 列表结束后的 `#` 重新参与；引用同理）；
- `  #`（2 空格）算文档级标题、`    #`（4 空格，缩进代码块）不算。
- **反向验证**：把闭合判定改回"剥离容器前缀"、把标题改回"必须顶格"后，正好这 3 条失败；恢复后 23/23。

### 9.5 [P2] 闭合围栏要先扣除所属容器的缩进 —— 已修

**问题**：虽然记了容器内容列，闭合判定仍对 `ctx.rest` 直接用只允许 0–3 空格的 `FENCE_PATTERN`，
于是列表项里合法闭合的 4 空格闭合行被拒（`10. ```js` / 4 空格内容 / 4 空格闭合），
围栏不闭合 → 后面的 `# Real` 被当成代码。

**修复**：闭合判定**先把所属容器的内容列从行首扣掉**，剩下的缩进再套"合法闭合围栏允许 0–3 空格"：
...
（`containerColumn` = 列表项的内容起始列，顶层 / 引用里为 0；顶层围栏里 4 空格的行依旧不是闭合围栏。）

### 9.6 [P2] 容器结束后未闭合的围栏必须结束 —— 已修

**问题**：围栏一直保留到文末，把容器外面的正文也吞进代码块。
「未闭合围栏延续到文末」只适用于**容器仍然成立**的情况。

**修复**：每行先判容器是否还成立 —— 引用里出现不带 `>` 的行（含空行，CommonMark 里空行结束引用）、
或列表项里出现缩进掉到内容列之前的非空行 —— 此时围栏以**上一行**为结尾结束，本行按"围栏外"继续处理
（它可能正是真正的标题）。

**用现有解析器对照这两个样例**（`sourceFoldingParse.test.ts`，现 **6/6**，断言代码块内容 + 文档级标题）：

| 样例                                             | 解析器给出的代码块内容                | 解析器给出的文档级标题 |
| ------------------------------------------------ | ------------------------------------- | ---------------------- |
| `10. ```js` / 4 空格内容 / 4 空格闭合 / `# Real` | `const x = 1`（闭合行是第 3 行）      | `# Real` ✓             |
| `> ```js` / `> const x = 1` / 空行 / `# Real`    | `const x = 1`（引用结束即代码块结束） | `# Real` ✓             |

扫描器侧的回归（`sourceFolding.test.ts` 现 **27/27**，新增 4 条）：
上面两个样例的**代码块结束行**与**后续标题恢复识别**都断言；
另加"引用里的空行带 `>` 时容器仍成立 → 围栏继续到真正的闭合行"与"列表项在围栏未闭合时就结束 → 围栏跟着结束"。
**反向验证**：把"扣容器列"与"容器结束即结束围栏"分别改回旧写法后，正好这 3 条失败；恢复后 33/33 通过
（27 扫描器 + 6 解析器对照）。
...
| 项目         | 覆盖                                                                                                  |
| ------------ | ----------------------------------------------------------------------------------------------------- |
| 表格边界删除 | 单测 1 条 + E2E `表格后` 场景（落点 `boundaryKind: "table"` → 第二下只删表格 → 撤销 → 重做 → 再撤销） |
| 嵌套容器     | 单测 4 条 + E2E `引用内嵌套` 场景（见 9.1）                                                           |
| 重做         | 所有剧本（提示块 / 图片 / 代码块 / 代码组 / 嵌套 / 表格）在撤销后都补了 `Cmd+Shift+Z` 断言            |

真实硬件长按速率与 `fn+Backspace` 真机映射仍明确列为**人工验证边界**（见 5 节）。
