...
- **定位方式**：源码视图用精确源码范围（偏移 + 逐字文本，标 `draft` / `disk`）；
  可视化视图用编辑器模型里的 PM 文档位置 + **该块在笔记源码文本里的偏移范围**
  （源码文本就是磁盘 / store 那一份）。块身份按**结构**对应，不靠文字搜索：

  - 顶层结构：PM 顶层节点 ↔ 源码扫描出的块**按序号一一对应**（块的**类型**也要对得上），
    数量或类型对不上直接拒绝固定；
  - 块内也是**结构映射**，不是文字搜索：按内容节点逐个推进 —— 文本节点的文字必须从当前位置
    起逐字命中，图片、行内公式、HTML、硬换行等按各自语法整体跳过，跳过的部分只能是标记或
    **不参与正文的区间**（链接 / 图片的地址与标题、HTML 标签）。所以
    `[x](AAA) AAA` 里选中链接后面的 `AAA` 落的是偏移 9，而不是链接地址里的 4；
  - 代码块用结构化围栏正文 + CodeMirror 自己的偏移；代码组面板按**面板序号**数第几个围栏；
  - 因此"文档里有两段一模一样的文字""同一个代码块里两个相同的词""不同面板里的相同代码"
    "链接地址 / 图片 alt 里有同名文字"都不会指错位置；这也意味着 **A 前插入内容导致坐标变化**、
    **原 A 被删除但别处还有相同文字**都会失效（不是全文搜索）。

- **拿不到可靠位置就拒绝固定**：多选区、空选区、结构对不上（例如文档有还没能写回源码的改动，
  排版结构与源码已经不一致），或者块内某段结构映射不出来（escaping 之外的 HTML 实体、
  认不出的行内节点等）都会明确拒绝，并把原因说清楚（`editor.hasUnsavedChanges` 为真时
  提示"位置锚不可信"）——**不生成"看起来有效、其实指错位置"的范围，也不留半份固定**。
- **代码块 / 代码组面板里的选区**：普通代码块的 CodeMirror 选区变化会上报（与代码组面板走同一条
  桥），所以"在代码块里选一段"就是那段，而不是整个代码块；同视图校验按 CodeMirror 偏移精确比对，
  跨视图 / 磁盘复核落到"源码文本里对应该块的那段选区范围"（范围就是选区本身，不是整块）。
- **跨视图 / 磁盘的精度**：位置锚是**选区级**的 —— 选区内部变化（含选区里的行内标记变化）
  失效；同一块里**选区之外**的后缀变化保留。注意在固定位置**之前**改动长度（哪怕随后改回来）
  会先出现一次"坐标变了"的中间状态，固定会因此失效：这是"坐标变化即失效"的既定规则。
...
| 可视化   | 普通代码块内部（CodeMirror 选区）                                | 文本 + 该代码块；**固定时精确到 CodeMirror 选区**（跨视图 / 磁盘复核退到块级） |
