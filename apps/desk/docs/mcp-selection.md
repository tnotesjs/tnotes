...
- **定位方式**：源码视图用精确源码范围（偏移 + 逐字文本，标 `draft` / `disk`）；
  ...
  - 顶层结构：PM 顶层节点 ↔ 源码扫描出的块**按序号一一对应**，数量对不上直接拒绝固定；
  - 块内：普通文本用块内纯文本偏移、代码块用结构化围栏正文 + CodeMirror 自己的偏移，
    代码组面板按**面板序号**数第几个围栏；
  - 因此"文档里有两段一模一样的文字""同一个代码块里两个相同的词""不同面板里的相同代码"
    都不会指错位置；这也意味着 **A 前插入内容导致坐标变化**、**原 A 被删除但别处还有相同文字**
    都会失效（不是全文搜索）。

- **拿不到可靠位置就拒绝固定**：多选区、空选区、结构对不上（例如文档有还没能写回源码的改动，
  排版结构与源码已经不一致）都会明确拒绝，并把原因说清楚（`editor.hasUnsavedChanges` 为真时
...
- **代码块 / 代码组面板里的选区**：普通代码块的 CodeMirror 选区变化会上报（与代码组面板走同一条
  ...
  跨视图 / 磁盘复核退到"涉及块"级（源码文本里对应该块的那段范围）。
