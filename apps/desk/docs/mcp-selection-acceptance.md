...
- **定位判据（两轮复核后重做，见下一小节）**：
  - **位置锚（跨视图 / 磁盘复核的唯一判据）**：偏移 + 期望文本，且**落在"笔记源码文本"的
    坐标系**里（不是序列化结果，也不是可视化视图自己编的行列）：
    - 源码固定：一个范围 = 选区本身；
    - 可视化固定：**每个涉及的块一个范围**。做法是从上到下**顺序**定位每个块在该笔记源码
      文本里的位置（下一块只从上一块结束处往后找），再在每个块里定位选区那一段；
      代码块 / 代码组面板内部用 CodeMirror 自己的选中文字；
  - 校验只比这些范围上的文字（**每个范围都要对上**），**不做全文搜索** ——
    所以"A 前插入内容让坐标变化""跨段落时任意一段变化""原 A 被删除而别处仍有相同文字"
    都会失效，而"同一块里选区之外的后缀变化"不会；
  - **可视化同视图校验**：块位置 + 类型 + **选区文本**（`textBetween`），
    只有整块选中（NodeSelection）才逐字比块源码；代码块 / 代码组面板内部用
    **CodeMirror 自己的坐标**比对。同一段落（或同一代码块）里**选区后方**的修改不再误伤；
  - **没有位置锚就拒绝固定**：拿不到可靠位置时明确提示"无法固定"，
    不会固定一份以后验不了、只能靠全文搜索猜位置的上下文。
- **仍存在的限制**：位置锚覆盖"第一个相关块的整块范围"，
  跨视图 / 磁盘复核时不区分"改动在选区内部还是同一块内的选区后方"（同视图内是精确的）；
  可视化锚点用的是"序列化文档坐标系"，若某个块在整篇序列化里的形态与它在源码里的形态差异很大，
  可能拿不到位置锚 —— 这时会**拒绝固定**而不是退化。
...
- **P1（跨段落固定只覆盖第一段）**：`blockTextRange()` 只用 `blockAnchors[0].markdown`
  的长度，所以"第二段 BBB 改成 CCC"在跨视图 / 磁盘路径仍判有效。
- **P2（同段落未选中后缀变化被误判失效）**：位置锚包含整个第一段而不是选中的那段文字，
  所以"只固定 AAA、后面 BBB 改成 CCC"在跨视图 / 磁盘路径会误失效。
- **修法（从锚点生成处改，不再调切片比较）**：可视化固定改为**按块产出多个位置范围**
  （每块一个，顺序定位在该笔记源码文本里；块内再定位选区那一段），校验时**每个范围都要对上**；
  同时**删掉"整篇序列化文本"这套坐标**（它与磁盘文本不天然共享偏移），
  位置锚统一落在**笔记源码文本**上 —— 可视化编辑器建出来用的就是这份文本，磁盘复核读的也是它。
- **真实采集覆盖**（不再只测"给定正确锚点切片有效"）：`pinnedAnchorCheck.test.ts`
  用真实编辑器采集锚点后断言 —— 跨段落（两段都覆盖：第二段变 → 失效）/
  同段落只固定 AAA（后缀 BBB→CCC → 保留；A 自身变、前方插入、别处顶替 → 失效）/
  同一代码块只固定 `const`（后缀变 → 保留、内部变 → 失效）/
  可视化同视图内第二段变化 → 失效；`pinnedContextService.test.ts` 补多范围磁盘复核
  （第二段变 → 失效；选区外追加 → 保留）。

### 第一轮复核后修掉的两点（P1 / P2）

- **P1（跨视图 / 磁盘复核退化成全文搜索）**：原实现里
  `NoteTabPane` 的兜底是 `content.includes(expected)`、`pinnedContextService.revalidateAgainst`
  是 `content.includes(block.markdown)` —— 在磁盘内容前插一段后仍返回 `pinned`，
  且"原位置被删、别处有相同文字"可能顶替。现在统一改成**位置锚 `textRange`** 的偏移切片校验，
  并把 `textRange` 变成**固定时的硬要求**（拿不到就明确拒绝固定）。
  单测：`pinnedAnchorCheck.test.ts` 的位置判据 5 条 + `pinnedContextService.test.ts`
  的磁盘位置复核（移位 / 重复文字 / 后方修改 / 草稿跳过）。
  E2E：可视化固定 → 切源码 → 前方插入 → 失效；外部修改 A（错位）→ 磁盘复核失效；
  原位置被删除但别处有相同文字 → 仍然失效；单纯切视图保留。
- **P2（可视化按整块比较，误伤未影响选区的修改）**：`validateVisualAnchor` 不再比较整块
  Markdown（NodeSelection 除外），改比**选区文本**（`textBetween`）+ 块位置/类型；
  代码块内部用 CM 自己的坐标。
  单测：同段落选区后方修改保留 / 内部修改失效 / 前方插入失效 / 跨段落；
  code 锚点（后方追加保留、内部改动失效、拿不到 CM 时不假装有效）。
  E2E：同一段落里选区后方修改保留 + 内部修改失效。
...
12. **可视化锚点的跨视图校验**：已改为位置锚（偏移 + 期望文本），不再是全文搜索；
    但它覆盖"第一个相关块的整块范围"，跨视图时不区分"改动在选区内部还是同一块内选区后方"
    （同视图内是精确的）。代码块 / 特殊块内部"选区后方修改保留"只有单测覆盖
    （E2E 里驱动 CM 光标依赖焦点与内部坐标，脆弱，未纳入）。
