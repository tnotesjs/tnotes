...
**`e2e-mcp-pinned-context.mjs`（本机 MCP 固定上下文，37/37）** 与
...
- **证据**：`pinnedContextService.test.ts` 11 条（快照/替换/拒绝与保留/校验失效/旧 pinId 竞态/
  ...
  （`⌘K P`、`⌘K V` 映射）；E2E `e2e-mcp-pinned-context.mjs` **37/37** 走真实 MCP 协议，
...
3. **跨视图 / 磁盘复核只到"块级"**：它只保证"涉及块在源码文本里的那段范围"没变，
   不区分改动落在选区内部还是同一块内的选区后方（同视图内是按 PM / CodeMirror 坐标精确的，
   有单测）；E2E 在跨视图路径上验的是"前方插入 → 失效"，"同块后方修改 → 保留"只在可视化
   同视图路径与单测里验过。
4. **代码组面板的精细定位**：多面板里"相同代码"的区分由 `pinnedAnchorCheck.test.ts`
   用**真实采集**验（`panelIndex=1` 落到第二个围栏）；E2E 没有构造多面板代码组，
   只验了普通代码块。
5. **写不回源码的草稿状态无法固定**：这是有意的拒绝（E2E 正面验过文案与"不留半份固定"），
   但"用户如何把文档恢复成可固定状态"（撤销那次编辑 / 重开笔记）没有做成引导入口。
6. **非正文输入的其它形态**：只验了侧栏搜索框；图片描述框、callout 标题输入框等按设计不触发
   （不在 `.milkdown` 里、也不是 CM 焦点），未逐个真实界面验证。
7. **块数 / 块字符超限**：只有单测覆盖；真实界面验的是选字超限。
8. **打包产物**：E2E 跑的是 `out/`（electron-vite build）；未跑 `e2e-packaged-smoke`，
   asar / 打包后 SDK 打进来是否正常未验（SDK 已进 main bundle，未在 .app 里跑过）。
9. **多窗口 / 多实例**：未验（本轮都是单窗口）。
10. **Windows / Linux**：未验（本轮只在 macOS）；服务本身与平台无关，但凭据存储与打包差异未测。
11. **端口占用后的客户端侧表现**：Desk 侧文案明确（不偷偷换端口），客户端如何报错未验。
12. **令牌轮换后的客户端重连**：Desk 侧旧令牌 401 + 旧会话断开已验证；客户端重连行为未验。
13. **固定上下文的快捷键**：物理按键（`⌘K P` / `⌘K V`）只能由 `tabShortcuts.test.ts` 钉住映射；
    E2E 无法驱动 Electron 的 `before-input-event`（Playwright 合成按键走 CDP），
    端到端只验到"命令 → 活动标签页固定/切换"这一段。
14. **`get_current_note` 与外部工具的真实联调**：只验到 MCP 协议层；真实 Agent 客户端读路径后
